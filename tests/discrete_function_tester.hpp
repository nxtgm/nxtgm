...
template <class T>
inline void
test_discrete_constraint_function(DiscreteConstraintFunctionBase *f) {
  ...
  // deserialize via factory function
  {
    ...
    for_each(shape, [&](auto labels) {
      ...
      if (!CHECK(is_value == doctest::Approx(should_value))) {
        ...
        std::cout << " -> IS " << is_value << " != SHOULD BE " << should_value
                  << std::endl;
      }
      ++i;
    });
...
inline void test_discrete_energy_function(DiscreteEnergyFunctionBase *f) {
  ...
  for_each(shape, [&](auto labels) {
    ...
    if (!CHECK(is_value == doctest::Approx(energies_copy[i]))) {
      std::cout << "ERROR: energies() ... consistent with copy_energies"
                << std::endl;
      for (auto i = 0; i < arity; ++i) {
        std::cout << labels[i] << " ";
      }
      std::cout << " -> IS " << is_value << " != SHOULD BE " << energies_copy[i]
                << std::endl;
    }
    ++i;
  });
