...
private:
  void print() {
    ...
    std::cout << fmt::format("{:5} {:<10} {:<10}", iteration_, dt_last,
                             dt_total);
    std::cout << fmt::format("{:<12.3e} ", lower_bound);
    std::cout << fmt::format("{:<12.3e} {:<6} {:<12.3e}   ", current.energy(),
                             current.is_feasible(), current.how_violated());
    std::cout << fmt::format("{:<12.3e} {:<6} {:<12.3e}\n", best.energy(),
                             best.is_feasible(), best.how_violated());
  }
