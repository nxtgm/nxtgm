...
template <class MODEL_TYPE, class DERIVED_OPTIMIZER_BASE> class OptimizerBase {
...
public:
  ...
  using reporter_callback_base_type =
      ReporterCallbackBase<DERIVED_OPTIMIZER_BASE>;
  using reporter_callback_wrapper_type =
      ReporterCallbackWrapper<reporter_callback_base_type>;

  using repair_callback_base_type = RepairCallbackBase<self_type>;
  using repair_callback_wrapper_type =
      RepairCallbackWrapper<repair_callback_base_type>;
  ...
  virtual SolutionValue best_solution_value() const {
    return this->model().evaluate(this->best_solution(),
                                  false /* early exit when infeasible*/);
  }
