...
{
    void ensure_size(std::size_t max_constraint_size,
...
public:
    ...
    virtual energy_type
    how_violated(const discrete_label_type* discrete_labels) const = 0;

    // convenience function
    virtual std::size_t size() const;
    virtual energy_type
    how_violated(std::initializer_list<discrete_label_type> labels) const;
...
{
public:
    inline DiscreteConstraintFunctionShape(
...
using DiscretConstraintFunctionSerializationFactory = std::unordered_map<
    std::string, std::function<std::unique_ptr<DiscreteConstraintFunctionBase>(
                     const nlohmann::json&)>>;
