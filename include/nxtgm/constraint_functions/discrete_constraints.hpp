...
{
public:
    static std::string serialization_key() { return "unique"; }
    using DiscreteConstraintFunctionBase::how_violated;
    virtual ~UniqueLables() {}

    UniqueLables(std::size_t arity, discrete_label_type n_labels,
    ...
    energy_type
    how_violated(const discrete_label_type* discrete_labels) const override;
    ...
    void add_to_lp(IlpData& ilp_data,
                   ...
                   IlpConstraintBuilderBuffer&) const override;

    nlohmann::json serialize_json() const override;

    static std::unique_ptr<DiscreteConstraintFunctionBase>
...
public:
    ...
    how_violated(const discrete_label_type* discrete_labels) const override;
    std::unique_ptr<DiscreteConstraintFunctionBase> clone() const override;
    void add_to_lp(IlpData&, const std::size_t*,
                   IlpConstraintBuilderBuffer&) const override;
    nlohmann::json serialize_json() const override;
    static std::unique_ptr<DiscreteConstraintFunctionBase>
    deserialize_json(const nlohmann::json& json);
