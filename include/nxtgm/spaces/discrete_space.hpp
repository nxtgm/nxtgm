...
public:
    ...
    inline std::size_t size() const { return n_variables_; }

    template <class F, class SOLUTION>
    void for_each_solution(SOLUTION& solution, F&& f) const
    {
        this->exitable_for_each_solution(solution,
                                         [&](const SOLUTION& s)
                                         {
                                             f(s);
                                             return true;
...
public:
    ...
    inline std::size_t num_indicator_variables() const { return n_variables_; }
        ...
        MODEL_SOLUTION& model_solution)
    {
        ...
        for (std::size_t vi = 0; vi < space_.size(); ++vi)
        {
            ...
            {
                const auto lp_sol =
                    indicator_variable_solution[mapping_begin + l];
                if (lp_sol > best)
                {
                    best = lp_sol;
                    best_label = l;
