...
{   
    ...
    auto export_optimizer(py::module_ & pymodule)
    {
        ...
        ;
        auto optimizer_cls = py::class_<optimzier_type, DiscreteGmOptimizerBase>(pymodule ,  optimzier_type::name().c_str(), py::dynamic_attr())
            .def(py::init<const DiscreteGm &, const parameters_type& >(), py::arg("gm"), py::arg("parameters") = parameters_type(),
                py::keep_alive<0, 1>()
            )
        ;
    ...
    void export_discrete_gm_optimizers(py::module_ & pymodule)
    {
        ...
        py::class_<DiscreteGmOptimizerBase>(pymodule, "DiscreteGmOptimizerBase")
            ...
            .def("best_solution", [](DiscreteGmOptimizerBase * optimizer)
            {
                const auto & sol = optimizer->best_solution();
                std::cout<<sol.size()<<std::endl;
                xt::pytensor<discrete_label_type, 1> array = xt::zeros<discrete_label_type>({uint16_t(optimizer->model().num_variables())});
                std::copy(sol.begin(), sol.end(), array.begin());
                return array;
            })
            .def("current_solution", [](DiscreteGmOptimizerBase * optimizer)
            {
