...
auto export_optimizer(py::module_& pymodule)
{
    ...
    auto parameters_cls =
        py::class_<parameters_type>(
            pymodule, (optimzier_type::name() + "Parameters").c_str())
            .def(py::init<>())
            .def_readwrite("time_limit", &parameters_type::time_limit);
    auto optimizer_cls =
        py::class_<optimzier_type, DiscreteGmOptimizerBase>(
            pymodule, optimzier_type::name().c_str(), py::dynamic_attr())
            .def(py::init<const DiscreteGm&, const parameters_type&>(),
                 py::arg("gm"), py::arg("parameters") = parameters_type(),
                 py::keep_alive<0, 1>());
...
void export_discrete_gm_optimizers(py::module_& pymodule)
{
    ...
    using reporter_callback_base_type =
        DiscreteGmOptimizerBase::reporter_callback_base_type;
    py::class_<reporter_callback_base_type>(
        pymodule, "DiscreteGmOptimizerReporterCcallbackBase");

    using reporter_callback = ReporterCallback<DiscreteGmOptimizerBase>;
    py::class_<reporter_callback, reporter_callback_base_type>(
        pymodule, "DiscreteGmOptimizerReporterCallback")
    ...
    py::class_<DiscreteGmOptimizerBase>(pymodule, "DiscreteGmOptimizerBase")
        ...
        .def("optimize", [](DiscreteGmOptimizerBase* optimizer)
             { return optimizer->optimize(); })
        .def("optimize", [](DiscreteGmOptimizerBase* optimizer,
                            reporter_callback_base_type* reporter_callback)
        ...
        .def("best_solution",
             [](DiscreteGmOptimizerBase* optimizer)
             {
                 ...
                 xt::pytensor<discrete_label_type, 1> array =
                     xt::zeros<discrete_label_type>(
                         {uint16_t(optimizer->model().num_variables())});
        ...
        .def("best_solution_value",
             &DiscreteGmOptimizerBase::best_solution_value)
        .def("current_solution_value",
             &DiscreteGmOptimizerBase::current_solution_value)
    ...
    export_optimizer<IlpHighs>(pymodule)
        ...
        .def_readwrite("highs_log_to_console",
                       &IlpHighs::parameters_type::highs_log_to_console);
