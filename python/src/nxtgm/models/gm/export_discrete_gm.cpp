...
void export_discrete_gm(py::module_ &pymodule) {
  py::class_<DiscreteGm>(pymodule, "DiscreteGm")
      ...
      .def(py::init<std::size_t, discrete_label_type>(), py::arg("num_var"),
           py::arg("num_labels"))
      // general space constructor from numpy array

      // from xtensor python tensor
      .def(py::init(
               [](const xt::pytensor<discrete_label_type, 1> &n_labels_array) {
                 return new DiscreteGm(n_labels_array.begin(),
                                       n_labels_array.end());
               }),
           py::arg("n_labels"))
      ...
      .def_property_readonly(
          "space", [](const DiscreteGm &gm) { return gm.space(); },
          py::return_value_policy::reference_internal)

      .def_property_readonly(
          "max_factor_arity",
          [](const DiscreteGm &gm) { return gm.max_factor_arity(); })
      .def_property_readonly(
          "max_factor_size",
          [](const DiscreteGm &gm) { return gm.max_factor_size(); })
      .def_property_readonly(
          "max_constraint_arity",
          [](const DiscreteGm &gm) { return gm.max_constraint_arity(); })
      .def_property_readonly(
          "max_constraint_size",
          [](const DiscreteGm &gm) { return gm.max_constraint_size(); })

      // evaluate from xt::pytensor
      .def(
          "evaluate",
          [](const DiscreteGm &gm,
             const xt::pytensor<discrete_label_type, 1> &labels,
             bool early_exit_infeasible) {
            span<const discrete_label_type> labels_span(labels.data(),
                                                        labels.size());
