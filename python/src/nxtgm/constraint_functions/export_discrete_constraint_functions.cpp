...
void export_discrete_constraint_functions(py::module_& pymodule)
{
    ...
    py::class_<DiscreteConstraintFunctionBase>(pymodule,
                                               "DiscreteConstraintFunctionBase")
    ...
    py::class_<UniqueLables, DiscreteConstraintFunctionBase>(pymodule,
                                                             "UniqueLables")
        .def(py::init<std::size_t, discrete_label_type, energy_type>(),
             py::arg("arity"), py::arg("num_labels"), py::arg("scale") = 1.0);

    py::class_<ArrayDiscreteConstraintFunction, DiscreteConstraintFunctionBase>(
        pymodule, "ArrayDiscreteConstraintFunction")
        // custom init from numpy via xt::pyarray from labmda
        .def(py::init([](xt::pyarray<energy_type> a)
                      { return new ArrayDiscreteConstraintFunction(a); }));
}
