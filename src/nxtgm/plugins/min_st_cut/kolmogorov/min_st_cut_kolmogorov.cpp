...
class MinStCutKolmogorov : public MinStCutBase
{
  public:
    ...
    double solve(int *uint8_t) override;
...
{
    max_flow_ = std::make_unique<max_flow_graph_type>(num_nodes_, num_edges_);
...
double MinStCutKolmogorov::solve(int *uint8_t)
{

    const double flow = max_flow_->maxflow();
    for (std::size_t i = 0; i < num_nodes_; ++i)
    {
        uint8_t[i] = max_flow_->what_segment(i) == max_flow_graph_type::SOURCE ? 0 : 1;
    }
    return flow;
}
