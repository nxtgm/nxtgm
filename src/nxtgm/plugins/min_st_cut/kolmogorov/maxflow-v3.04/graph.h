...
class Graph
{
  public:
    ...
    // 1, and so on. If num>1, then several nodes are added, and node_id of the first one is returned. IMPORTANT: see
  ...
  public:
    ...
    //  - In the example above, the user may omit calls g->remove_from_changed_list(i) and changed_list->Reset() in a
    //  given iteration.
...
template <typename captype, typename tcaptype, typename flowtype>
inline typename Graph<captype, tcaptype, flowtype>::termtype Graph<captype, tcaptype, flowtype>::what_segment(
    node_id i, termtype default_segm)
{
    if (nodes[i].parent)
    {
