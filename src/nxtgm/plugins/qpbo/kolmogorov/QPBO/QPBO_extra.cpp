...
#define SET_SISTERS(a, a_rev)                                                                                          \
    (a)->sister = (a_rev);                                                                                             \
    (a_rev)->sister = (a);
#define SET_FROM(a, i)                                                                                                 \
    (a)->next = (i)->first;                                                                                            \
    (i)->first = (a);
#define REMOVE_FROM(a, i)                                                                                              \
    if ((i)->first == (a))                                                                                             \
        (i)->first = (a)->next;                                                                                        \
    else                                                                                                               \
    {                                                                                                                  \
        Arc *a_TMP_REMOVE_FROM;                                                                                        \
        for (a_TMP_REMOVE_FROM = i->first;; a_TMP_REMOVE_FROM = a_TMP_REMOVE_FROM->next)                               \
            if (a_TMP_REMOVE_FROM->next == (a))                                                                        \
            {                                                                                                          \
                a_TMP_REMOVE_FROM->next = (a)->next;                                                                   \
                break;                                                                                                 \
            }                                                                                                          \
    }
...
int QPBO<REAL>::MergeParallelEdges(Arc *a1, Arc *a2)
{
    ...
    else
    {
        ...
        // make sure that _a1[0]->r_cap == _a1[1]->r_cap and _a1[0]->sister->r_cap == _a1[1]->sister->r_cap by pushing
        // flow
        delta = _a1[1]->r_cap - _a1[0]->r_cap;
        //_a1[1]->r_cap -= delta;   // don't do the subtraction - later we'll set explicitly _a1[1]->r_cap =
        //_a1[0]->r_cap _a1[1]->sister->r_cap += delta;
        _a1[1]->sister->head->tr_cap -= delta;
        _a1[1]->head->tr_cap += delta;
        // same for a2
        delta = _a2[1]->r_cap - _a2[0]->r_cap;
        //_a2[1]->r_cap -= delta;   // don't do the subtraction - later we'll set explicitly _a2[1]->r_cap =
        //_a2[0]->r_cap _a2[1]->sister->r_cap += delta;
        _a2[1]->sister->head->tr_cap -= delta;
        _a2[1]->head->tr_cap += delta;

        if (a1->r_cap + a1->sister->r_cap >= a2->r_cap + a2->sister->r_cap)
            x = 1;
        else // swap a1 <-> a2
        {
            Arc *tmp;
...
inline bool QPBO<REAL>::AddDirectedConstraint0(Arc *a, int xi, int xj)
{
    ...
    if (xi == 0)
        a->r_cap += probe_options.C + a->sister->r_cap - a->r_cap;
    else
        a->sister->r_cap += probe_options.C + a->sister->r_cap - a->r_cap;
...
inline bool QPBO<REAL>::AddDirectedConstraint1(Arc *a, int xi, int xj)
{
    ...
    {
        if (a->sister->r_cap > 0 && _a[1]->sister->r_cap > 0)
            return false;
        mark_node(_j[0]);
        mark_node(_j[1]);
        a->sister->r_cap += probe_options.C;
...
void QPBO<REAL>::AllocateNewEnergy(int *mapping)
{
    ...
    for (i_index = 0, i = nodes[0]; i_index < nodeNumOld; i_index++, i++)
    {
        ...
        else if (mapping[i_index] >= 2)
            mapping[i_index] = -mapping[i_index];
    }
    ...
    for (e = 0; e < edgeNumOld; e++)
    {
        if (arcs[0][2 * e].sister)
        {
            ...
            if (IsNode0(a->head))
            {
                ...
                AddPairwiseTerm(i_index, j_index, 0, a->r_cap + a_mate->r_cap, a->sister->r_cap + a_mate->sister->r_cap,
                                0);
            }
            else
            {
                ...
                code_assert(j_index > 0 && j_index < nodeNumNew);
                AddPairwiseTerm(i_index, j_index, a->r_cap + a_mate->r_cap, 0, 0,
                                a->sister->r_cap + a_mate->sister->r_cap);
            }
        }
    }
...
{
    if (num > GetMaxEdgeNum())
        reallocate_arcs(2 * num);
}
...
bool QPBO<REAL>::Probe(int *mapping)
{
    ...
    if (probe_options.order_array)
        probe_options.order_seed = 0;
    ...
    if (node_last[0] == node_max[0])
        reallocate_nodes((int)(node_last[0] - nodes[0]) + 1);
    ...
    while (1)
    {
        ...
        for (i_index = list.GetNext(-current_list); i_index >= 0; i_index = i_index_next)
        {
            ...
            for (ptr = changed_list->ScanFirst(); ptr; ptr = changed_list->ScanNext())
            {
                ...
                if (j->label_after_fix0 == what_segment(GetMate0(j)))
                    j->label_after_fix0 = -1;
                else if (i->user_label == 0)
                    j->user_label = j->label_after_fix0;
            }
            ...
            for (ptr = changed_list->ScanFirst(); ptr; ptr = changed_list->ScanNext())
            {
                ...
                if (j->label_after_fix1 == what_segment(GetMate0(j)))
                    j->label_after_fix1 = -1;
                else if (i->user_label == 1)
                    j->user_label = j->label_after_fix1;

                if (i == j || j->label_after_fix0 < 0 || j->label_after_fix1 < 0)
                    continue;
                ...
                j->is_removed = true;
                if (i_index_next == j_index)
                    i_index_next = list.GetNext(j_index);
                list.Remove(j_index);
                unlabeled_num--;
            }
            ...
            for (a = i->first; a; a = a->next)
            {
                ...
                for (x = 0; x < 2; x++)
                {
                    ...
                    {
                        if (AddDirectedConstraint0(a, x, label[x]))
                            is_changed = true;
                    }
                }
            }
            ...
            for (a = i->first; a; a = a->next)
            {
                ...
                for (x = 0; x < 2; x++)
                {
                    if (label[x] >= 0 && label[1 - x] < 0)
                    {
                        if (AddDirectedConstraint1(a, x, label[x]))
                            is_changed = true;
                    }
                }
            }
            ...
            {
                // if is_changed, add i and its neighbors to list 3, otherwise add i to list 2 (unless it's already
                // there)
                if (is_changed)
                {
            ...
            for (ptr = changed_list->ScanFirst(); ptr; ptr = changed_list->ScanNext())
            {
                ...
                if (j->label == what_segment(GetMate0(j)))
                    j->label = -1;
