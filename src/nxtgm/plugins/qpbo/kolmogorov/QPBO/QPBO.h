...
class QPBO
{
  public:
    ...
    void SetMaxEdgeNum(EdgeId num); // If num > edge_num_max then memory for edges is reallocated. Important for Probe()
                                    // with option=1,2.

    ///////////////////////////////////////////////////////////////

    // Adds node(s) to the graph. By default, one node is added (num=1); then first call returns 0, second call returns
    // 1, and so on. If num>1, then several nodes are added, and NodeId of the first one is returned. IMPORTANT: see
    ...
    // NOTE: if the energy is submodular, then ComputeWeakPersistences() will label all nodes (in general, this is not
    // necessarily true for Solve()).
    void ComputeWeakPersistencies();

    // GetRegion()/Stitch():
    // ComputeWeakPersistencies() also splits pixels into regions (``strongly connected components'') U^0, U^1, ..., U^k
    // as described in
    //
    ...
    struct ProbeOptions
    {
        ...
        int directed_constraints; // 0: directed constraints are added only for existing edges
                                  // 1: all possible directed constraints are added, if there is sufficient space for
                                  // edges (as specified by edge_num_max; see SetEdgeNumMax() function) 2: all possible
                                  // directed constraints are added. If necessary, new memory for edges is allocated.
        ...
        int *order_array; // if array of size nodeNum() is provided, then nodes are tested in the order order_array[0],
                          // order_array[1], ...
        ...
        bool (*callback_fn)(int unlabeled_num); // if callback_fn!=NULL, then after every testing a node Probe calls
                                                // callback_fn(); unlabeled_num is the current number of remaining nodes
                                                // in the energy. If callback_fn returns true then Probe() terminates.
    };
...
#pragma pack()
    ...
    Node *GetMate(Node *i)
    {
        return IsNode0(i) ? GetMate0(i) : GetMate1(i);
    }
    bool IsArc0(Arc *a)
    {
        return (a < arcs[1]);
    ...
    Arc *GetMate(Arc *a)
    {
        return IsArc0(a) ? GetMate0(a) : GetMate1(a);
    }
    ...
    int MergeParallelEdges(
        Arc *a1, Arc *a2); // there must hold (a1->sister->head == a2->sister->head) && IsNode0(a1->sister->head) &&
    ...
    bool AddDirectedConstraint1(
        Arc *a, int xi, int xj); // ...0 checks whether submodurality needs to be swapped, ...1 preserves submodularity.
...
inline void QPBO<REAL>::GetTwiceUnaryTerm(NodeId i, REAL &E0, REAL &E1)
{
    ...
    E0 = 0;
    if (stage == 0)
        E1 = 2 * nodes[0][i].tr_cap;
    else
        E1 = nodes[0][i].tr_cap - nodes[1][i].tr_cap;
}
...
template <typename REAL>
inline void QPBO<REAL>::ComputeWeights(REAL A, REAL B, REAL C, REAL D,          // input - E00=A, E01=B, E10=C, E11=D
