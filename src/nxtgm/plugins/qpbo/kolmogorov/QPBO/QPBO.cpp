      ...
      zero_energy(q.zero_energy)
{
    ...
    for (i = nodes[0]; i < node_last[stage]; i++)
    {
        if (i == node_last[0])
            i = nodes[1];
        if (i->first)
            i->first = (Arc *)((char *)i->first + (((char *)arcs[0]) - ((char *)q.arcs[0])));
    }
    ...
    for (a = arcs[0]; a < arc_max[stage]; a++)
    {
        ...
        if (a->sister)
        {
            ...
            if (a->next)
                a->next = (Arc *)((char *)a->next + (((char *)arcs[0]) - ((char *)q.arcs[0])));
...
void QPBO<REAL>::reallocate_arcs(EdgeId arc_num_max_new)
{
    ...
    EdgeId arc_num_max = arc_num_max_new;
    if (arc_num_max & 1)
        arc_num_max++;
...
void QPBO<REAL>::TransformToSecondStage(bool copy_trees)
{
    ...
    else
    {
        for (i[0] = nodes[0], i[1] = nodes[1]; i[0] < node_last[0]; i[0]++, i[1]++)
        {
            ...
            if (i[0]->parent == NULL || i[0]->parent == QPBO_MAXFLOW_TERMINAL)
                i[1]->parent = i[0]->parent;
            else
                i[1]->parent = GetMate0(i[0]->parent->sister);
        }
...
void QPBO<REAL>::TestRelaxedSymmetry()
{
    ...
    for (i = nodes[0]; i < node_last[0]; i++)
    {
        ...
        for (a = GetMate0(i)->first; a; a = a->next)
            c2 += a->r_cap;
        if (c1 != c2)
        {
            code_assert(0);
