...
void QPBO<REAL>::maxflow(bool reuse_trees, bool _keep_changed_list)
{
    ...
    {
        if (!changed_list)
            changed_list = new Block<Node *>(NODEPTR_BLOCK_SIZE, error_function);
    }
...
void QPBO<REAL>::test_consistency(Node *current_node)
{
    ...
    for (i = nodes[0]; i < node_last[stage]; i++)
    {
        ...
        {
            if (!i->is_sink)
                code_assert(i->parent->sister->r_cap > 0);
            else
        ...
        if (i->parent && !i->next)
        {
            if (!i->is_sink)
            {
                ...
                {
                    if (a->r_cap > 0)
                        code_assert(a->head->parent && !a->head->is_sink);
                }
            }
            else
            {
        ...
        if (i->parent && i->parent != QPBO_MAXFLOW_ORPHAN && i->parent != QPBO_MAXFLOW_TERMINAL)
        {
            ...
            if (i->TS == i->parent->head->TS)
                code_assert(i->DIST > i->parent->head->DIST);
        }
    }
}
