...
class ReducedGmOptimizer : public DiscreteGmOptimizerBase
{
  ...
  public:
    ...
    ReducedGmOptimizer(const DiscreteGm &gm, const OptimizerParameters &parameters);
  ...
  private:
    OptimizationStatus get_partial_optimality(OptimizerTimer &, reporter_callback_wrapper_type &,
                                              repair_callback_wrapper_type &);
    OptimizationStatus get_partial_optimality_via_qpbo(OptimizerTimer &, reporter_callback_wrapper_type &,
                                                       repair_callback_wrapper_type &);

    OptimizationStatus build_and_optimize_submodel(OptimizerTimer &, reporter_callback_wrapper_type &,
                                                   repair_callback_wrapper_type &);
...
class ReducedGmOptimizerFactory : public DiscreteGmOptimizerFactoryBase
{
  public:
    ...
    {
        return std::make_unique<ReducedGmOptimizer>(gm, params);
    }
...
ReducedGmOptimizer::ReducedGmOptimizer(const DiscreteGm &gm, const OptimizerParameters &parameters)
    : base_type(gm),
...
{
    // start the timer
    OptimizerTimer timer;

    // get the partial optimal variables
    OptimizationStatus status = this->get_partial_optimality(timer, reporter_callback, repair_callback);
    ...
    return this->build_and_optimize_submodel(timer, reporter_callback, repair_callback);
}
...
OptimizationStatus ReducedGmOptimizer::get_partial_optimality(OptimizerTimer &timer,
                                                              reporter_callback_wrapper_type &reporter_callback,
                                                              repair_callback_wrapper_type &repair_callback)
{
    return this->get_partial_optimality_via_qpbo(timer, reporter_callback, repair_callback);
}

OptimizationStatus ReducedGmOptimizer::get_partial_optimality_via_qpbo(
    OptimizerTimer &timer, reporter_callback_wrapper_type & /*reporter_callback*/,
    repair_callback_wrapper_type & /*repair_callback*/
)
{
...
OptimizationStatus ReducedGmOptimizer::build_and_optimize_submodel(OptimizerTimer &, reporter_callback_wrapper_type &,
                                                                   repair_callback_wrapper_type &)
{
    ...
    if (n_partial_optimal_ == 0)
    {
        ...
        auto optimizer = factory->create(gm, parameters_.sub_optimizer_parameters);
