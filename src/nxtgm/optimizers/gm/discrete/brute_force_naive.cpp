...
OptimizationStatus BruteForceNaive::optimize(
  ...
  this->model().space().exitable_for_each_solution(
      current_solution_, [&](const solution_type &solution) {
        // evaluate the current solution
        this->current_sol_value_ =
            this->model().evaluate(solution, early_stop_infeasible);
        ...
        if (this->current_sol_value_ < this->best_sol_value_) {
          ...
          // via `paused_call`
          if (reporter_callback && !timer.paused_call([&]() {
...
const typename BruteForceNaive::solution_type &
BruteForceNaive::best_solution() const {
  return this->best_solution_;
}
const typename BruteForceNaive::solution_type &
BruteForceNaive::current_solution() const {
