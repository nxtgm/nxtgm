...
DynamicProgramming::DynamicProgramming(const DiscreteGm& gm,
    ...
    : base_type(gm), parameters_(parameters),
      ...
      value_buffers_(gm.space().size()), state_buffers_(gm.space().size()),
      ...
      ordered_nodes_(gm.space().size(), std::numeric_limits<std::size_t>::max())
{
    if (gm.max_factor_arity() > 2)
    {
        throw std::runtime_error(
            "DynamicProgramming only supports factors of arity 2");
    }
    if (!gm.constraints().empty())
    {
        throw std::runtime_error(
            "DynamicProgramming does not support constraints");
    }
...
)
{
    ...
    std::vector<discrete_label_type> factor_label_buffer(gm.max_factor_size(),
                                                         0);
    ...
    for (std::size_t i = 1; i <= gm.num_variables(); ++i)
    {
        ...
        std::fill(value_buffers_[node],
                  value_buffers_[node] + gm.num_labels(node), energy_type(0));
        ...
        for (auto fid : factors_of_variables_[node])
        {
            ...
            if (factor.arity() == 2)
            {
                ...
                if (vars[0] == node && node_order_[vars[1]] > node_order_[node])
                {
                    ...
                    energy_type v;
                    for (discrete_label_type l0 = 0; l0 < gm.num_labels(node);
                         ++l0)
                    {
                        v = std::numeric_limits<energy_type>::infinity();
                        for (discrete_label_type l1 = 0;
                             l1 < gm.num_labels(node2); ++l1)
                        {
                            const auto factor_value = factor({l0, l1});
                            const auto v2 =
                                factor_value + value_buffers_[node2][l1];
                            if (v2 < v)
                            {
                                v = v2;
                                s = l1;
                        ...
                        state_buffers_[node]
                                      [children_counter * gm.num_labels(node) +
                                       l0] = s;
                ...
                if (vars[1] == node && node_order_[vars[0]] > node_order_[node])
                {
                    ...
                    for (discrete_label_type l1 = 0; l1 < gm.num_labels(node);
                         ++l1)
                    {
                        discrete_label_type s;
                        auto v = std::numeric_limits<energy_type>::infinity();

                        for (discrete_label_type l0 = 0;
                             l0 < gm.num_labels(node2); ++l0)
                        {

                            const auto v2 =
                                factor({l0, l1}) + value_buffers_[node2][l0];
                            if (v2 < v)
                            {
                                v = v2;
                                s = l0;
...
const typename DynamicProgramming::solution_type&
DynamicProgramming::best_solution() const
{
    return this->best_solution_;
}
const typename DynamicProgramming::solution_type&
DynamicProgramming::current_solution() const
{
    return this->best_solution_;
}
...
void DynamicProgramming::compute_labels()
{
    ...
    std::vector<std::size_t> node_list;
    std::fill(best_solution_.begin(), best_solution_.end(),
              std::numeric_limits<discrete_label_type>::max());
    std::size_t var = 0;
    while (var < gm.num_variables())
    {
        if (best_solution_[var] ==
            std::numeric_limits<discrete_label_type>::max())
        {
        ...
        while (node_list.size() > 0)
        {
            ...
            for (auto&& fid : factors_of_variables_[node])
            {
                ...
                {
                    if (vars[1] == node &&
                        node_order_[vars[0]] > node_order_[node])
                    {
                        best_solution_[vars[0]] =
                            state_buffers_[node][children_counter *
                                                     gm.num_labels(node) +
                                                 best_solution_[node]];
                    ...
                    if (vars[0] == node &&
                        node_order_[vars[1]] > node_order_[node])
                    {
