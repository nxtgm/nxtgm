...
OptimizationStatus DynamicProgramming::optimize(
  ...
  for (std::size_t i = 1; i <= gm.num_variables(); ++i) {
    ...
    std::fill(value_buffers_[node], value_buffers_[node] + gm.num_labels(node),
              energy_type(0));
      ...
      if (factor.arity() == 2) {
        ...
        if (vars[0] == node && node_order_[vars[1]] > node_order_[node]) {
          ...
          for (discrete_label_type l0 = 0; l0 < gm.num_labels(node); ++l0) {
            ...
            state_buffers_[node][children_counter * gm.num_labels(node) + l0] =
                s;
        ...
        if (vars[1] == node && node_order_[vars[0]] > node_order_[node]) {
          ...
          for (discrete_label_type l1 = 0; l1 < gm.num_labels(node); ++l1) {
            ...
            state_buffers_[node][children_counter * gm.num_labels(node) + l1] =
                s;
        ...
        if (factor.arity() == 2) {
          if (vars[1] == node && node_order_[vars[0]] > node_order_[node]) {
            best_solution_[vars[0]] =
                state_buffers_[node][children_counter * gm.num_labels(node) +
                                     best_solution_[node]];
