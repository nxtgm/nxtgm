  ...
  for (const auto &constraint : constraints_) {
    ...
    if (how_violated >= constraint_feasiblility_limit) {
      if (early_stop_infeasible) {
        return SolutionValue{std::numeric_limits<energy_type>::infinity(),
                             how_violated};
      } else {
...
nlohmann::json DiscreteGm::serialize_json() const {
  ...
  // for the constraints we need to figure out the index of the function in the
  // constrain wrt the vector
