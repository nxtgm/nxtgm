...
DiscreteGm::DiscreteGm(const DiscreteSpace& discrete_space)
    : space_(discrete_space), factors_(), energy_functions_(), constraints_(),
      constraint_functions_(), max_factor_arity_(0), max_constraint_arity_(0),
      max_factor_size_(0), max_constraint_size_(0)
{
}

std::size_t DiscreteGm::add_energy_function(
    std::unique_ptr<DiscreteEnergyFunctionBase> function)
{
    energy_functions_.push_back(std::move(function));
    return energy_functions_.size() - 1;
}

std::size_t DiscreteGm::add_constraint_function(
...
SolutionValue DiscreteGm::evaluate(const solution_type& solution,
                                   bool early_stop_infeasible) const
{
    span<const discrete_label_type> solution_span(solution.data(),
                                                  solution.size());
...
nlohmann::json DiscreteGm::serialize_json() const
{
    ...
    // early)
    {
        std::unordered_map<const DiscreteEnergyFunctionBase*, std::size_t>
            energy_function_index_map;
        ...
        for (const auto& factor : factors_)
        {
            jfactors.push_back({{{"functuin_index",
                                  energy_function_index_map[factor.function()]},
    ...
    std::unordered_map<const DiscreteConstraintFunctionBase*, std::size_t>
        constraint_function_index_map;
    ...
    for (const auto& factor : constraints_)
    {
        jconstraints.push_back(
            {{{"functuin_index",
               constraint_function_index_map[factor.function()]},
              {"variables", factor.variables()}}});
    }
...
DiscreteGm DiscreteGm::deserialize_json(const nlohmann::json& json)
{
    ...
    for (const auto& jenergy_function : jenergy_functions)
    {
        auto energy_function =
            discrete_energy_function_deserialize_json(jenergy_function);
    ...
    for (const auto& jconstraint_function : jconstraint_functions)
    {
        auto constraint_function =
            discrete_constraint_function_deserialize_json(jconstraint_function);
    ...
    for (const auto& jconstraint : jconstraints)
    {
        ...
        auto variables =
            jconstraint["variables"].get<std::vector<std::size_t>>();
        gm.add_factor(variables, function_index);
    }
