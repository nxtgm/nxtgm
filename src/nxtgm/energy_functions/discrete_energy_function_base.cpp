...
void IlpFactorBuilderBuffer::ensure_size(std::size_t max_factor_size,
...
energy_type DiscreteEnergyFunctionBase::energy(
    std::initializer_list<discrete_label_type> discrete_labels) const
{
    ...
    IlpFactorBuilderBuffer& buffer) const
{
    ...
    this->copy_energies(buffer.energy_buffer.data(),
                        buffer.label_buffer.data());

    if (arity == 1)
    {
        for (discrete_label_type label = 0;
             label < static_cast<discrete_label_type>(factor_size); ++label)
        {
    ...
    else
    {
        ...
        auto factor_indicator_vars =
            xt::eval(xt::arange(start, start + factor_size)
                         .reshape(const_discrete_label_span(shape, arity)));

        ilp_data.add_variables(0.0, 1.0, energies, energies + factor_size,
                               false);
        ...
        for (auto ai = 0; ai < arity; ++ai)
        {
            ...
            {
                const auto var_inidcator =
                    indicator_variables_mapping[ai] + label;
                auto constraint_vars =
                    bind_at(factor_indicator_vars, ai, label);
...
std::pair<std::unique_ptr<DiscreteEnergyFunctionBase>, energy_type>
DiscreteEnergyFunctionBase::bind(
