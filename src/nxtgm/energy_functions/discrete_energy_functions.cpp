...
Potts::Potts(std::size_t num_labels, energy_type beta)
    : num_labels_(num_labels), beta_(beta) {}

std::size_t Potts::arity() const { return 2; }

discrete_label_type Potts::shape(std::size_t) const { return num_labels_; }

std::size_t Potts::size() const { return num_labels_ * num_labels_; }
...
nlohmann::json Potts::serialize_json() const {
  return {{"type", Potts::serialization_key()},
          {"num_labels", num_labels_},
          {"beta", beta_}};
}

std::unique_ptr<DiscreteEnergyFunctionBase>
Potts::deserialize_json(const nlohmann::json &json) {
...
std::size_t XArray::arity() const { return values_.dimension(); }

std::size_t XArray::size() const { return values_.size(); }

energy_type XArray::energy(const discrete_label_type *discrete_labels) const {
  const_discrete_label_span discrete_labels_span(discrete_labels,
                                                 values_.dimension());
...
void XArray::add_energies(energy_type *energies, discrete_label_type *) const {
  std::transform(values_.data(), values_.data() + values_.size(), energies,
                 energies, std::plus<energy_type>());
}
...
nlohmann::json XArray::serialize_json() const {
  ...
  return {{"type", XArray::serialization_key()},
          {"shape", shape},
          {"values", values}};
}

std::unique_ptr<DiscreteEnergyFunctionBase>
XArray::deserialize_json(const nlohmann::json &json) {
...
std::size_t LabelCosts::arity() const { return arity_; }

std::size_t LabelCosts::size() const { return std::pow(costs_.size(), arity_); }

energy_type
LabelCosts::energy(const discrete_label_type *discrete_labels) const {
...
void LabelCosts::add_to_lp(IlpData &ilp_data,
  ...
  for (std::size_t ai = 0; ai < arity_; ++ai) {
    ...
    for (discrete_label_type l = 0;
         l < static_cast<discrete_label_type>(costs_.size()); ++l) {

      ilp_data.begin_constraint(0.0, 1.0);
      ilp_data.add_constraint_coefficient(label_indicator_variables_begin + l,
                                          1.0);
      ilp_data.add_constraint_coefficient(indicator_variables_mapping[ai] + l,
                                          -1.0);
    }
  }
...
nlohmann::json LabelCosts::serialize_json() const {
  return {{"type", LabelCosts::serialization_key()},
          {"arity", arity_},
          {"values", costs_}};
}

std::unique_ptr<DiscreteEnergyFunctionBase>
LabelCosts::deserialize_json(const nlohmann::json &json) {
  return std::make_unique<LabelCosts>(json["arity"], json["values"].begin(),
                                      json["values"].end());
}
